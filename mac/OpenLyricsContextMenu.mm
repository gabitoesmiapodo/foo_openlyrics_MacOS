// mac/OpenLyricsContextMenu.mm
// Playlist context menu items for foobar2000 macOS.
// Mirrors src/ui_contextmenu.cpp but calls macOS-specific spawn functions.
#include "stdafx.h"
...
static contextmenu_group_popup_factory g_ctx_item_factory(GUID_OPENLYRICS_CTX_POPUP,
                                                          ...
                                                          "OpenLyrics");
static contextmenu_group_factory g_openlyrics_ctx_subgroup(GUID_OPENLYRICS_CTX_SUBGROUP,
                                                            GUID_OPENLYRICS_CTX_POPUP, 0);
...
public:
                             ...
                             const GUID& caller) override
    {
        ...
        // Disable & grey-out items that only apply to single selection
        if(data.get_count() > 1)
        {
            switch(index)
            {
    ...
    void context_command(unsigned int index, metadb_handle_list_cref data,
                         const GUID& /*caller*/) override
    {
        switch(index)
        {
            case cmd_show_lyrics:
            {
                ...
                const auto async_search = [track](threaded_process_status& /*status*/,
                                                   abort_callback& abort)
                {
                    ...
                    LyricSearchHandle handle(LyricUpdate::Type::InternalSearch,
                                             track, track_info, abort);
                    ...
                    if(success)
                    {
                        if(handle.has_result())
                        {
                            ...
                            const std::string dialog_contents = std::format(
                                ...
                                from_tstring(text));
                            popup_message::g_show(dialog_contents.c_str(),
                                                  dialog_title.c_str());
                        }
                        else
                        {
                    ...
                    {
                        popup_message::g_show("Failed to retrieve locally-saved lyrics",
                                              dialog_title.c_str());
                    }
                };

                threaded_process::g_run_modeless(
                    threaded_process_callback_lambda::create(async_search),
            ...
            case cmd_bulksearch_lyrics:
            {
                ...
                SpawnBulkLyricSearchMac(tracks);
            }
            break;
            ...
            case cmd_manualsearch_lyrics:
            {
                if(data.get_count() == 0) break;
                metadb_handle_ptr track = data.get_item(0);
                SpawnManualSearchMacForTrack(track, get_full_metadata(track));
            }
            break;
            ...
            case cmd_edit_lyrics:
            {
                ...
                const auto async_edit = [track](threaded_process_status& /*status*/,
                                                 abort_callback& abort)
                {
                    const metadb_v2_rec_t track_info = get_full_metadata(track);
                    LyricSearchHandle search_handle(LyricUpdate::Type::InternalSearch,
                                                    track, track_info, abort);
                    ...
                    if(success)
                    {
                        ...
                        {
                            SpawnLyricEditorMacForTrack(lyrics, track, track_info);
                        });
                    }
                };

                threaded_process::g_run_modeless(
                    threaded_process_callback_lambda::create(async_edit),
                    threaded_process::flag_show_delayed | threaded_process::flag_show_abort,
                    core_api::get_main_window(),
                    "Retrieving saved lyrics...");
            }
            ...
            case cmd_mark_instrumental:
            {
                ...
                const size_t track_count = data.get_count();
                std::string msg;
                if(track_count == 1)
                {
                    ...
                    msg += "and mark the track as instrumental. OpenLyrics will no longer search for "
                           "lyrics for this track automatically so it will not show any lyrics for "
                           "this track until you explicitly request a search for it."
                ...
                else
                {
                    ...
                    msg += " selected tracks and mark those tracks as instrumental. OpenLyrics will "
                           "no longer search for lyrics for those tracks automatically so it will "
                           "not show any lyrics for those tracks until you explicitly request a "
                           "search for it.\n\nAre you sure you want to proceed?";
                }
                ...
                const auto async_instrumental =
                    [data_copy, track_count](threaded_process_status& /*status*/,
                                             abort_callback& abort)
                {
                    ...
                    metadb_v2::ptr meta;
                    if(metadb_v2::tryGet(meta))
                    {
                        const auto fill_track_info = [&all_track_info](size_t idx,
                    ...
                    LOG_INFO("Marking %d tracks as instrumental from the context menu...",
                             int(track_count));
                    for(size_t i = 0; i < track_count; i++)
                    {
                        metadb_handle_ptr track     = data_copy.get_item(i);
                        const metadb_v2_rec_t& info = all_track_info[i];
                        ...
                        search_avoidance_force_by_mark_instrumental(track, info);
                    }
                    LOG_INFO("Finished marking %d tracks as instrumental with %d failures",
                             int(track_count), int(failure_count));
                    ...
                    fb2k::inMainThread2(
                        [failure_count, track_count]()
                        {
                            ...
                            if(failure_count == 0)
                            {
                                result_msg += "Successfully marked all ";
                                result_msg += std::to_string(track_count);
                                result_msg += " tracks as instrumental";
                            }
                            else
                            {
                                result_msg += "Failed to mark ";
                                result_msg += std::to_string(failure_count);
                                result_msg += " (of ";
                                result_msg += std::to_string(track_count);
                ...
                threaded_process::g_run_modeless(
                    threaded_process_callback_lambda::create(async_instrumental),
                    threaded_process::flag_show_delayed | threaded_process::flag_show_abort,
                    core_api::get_main_window(),
