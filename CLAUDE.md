...
- **Search-result matching** (`src/tag_util.cpp`, `src/lyric_io.cpp`): `tag_values_match` accepts tags within a length-proportional edit-distance threshold (base 3, scaled up for longer/multi-byte tags) plus a trailing edition/version-marker tolerance (case-insensitive prefix ending on a word boundary). On macOS it first folds CJK Traditional → Simplified via `fold_for_tag_match` (`mac/PlatformUtil.mm`; a different ICU transform from the NFKD normalisation above) so Traditional-tagged local tracks match Simplified source metadata. In `internal_search_for_lyrics` the album is not a hard filter: artist+title is the primary rule, with a title+album recovery rule for romanised-vs-native artist names; album still feeds ranking via `sort_source_results`.
- **`g_exists` across the binary boundary** (`src/tag_util.cpp` `g_exists_safe`): `filesystem::g_exists` can throw `exception_io_not_found` across the foobar2000.app/component boundary instead of returning `false` (typeinfo mismatch), so existence checks route through `g_exists_safe`, which treats a throw as "does not exist". Auto-search is skipped for missing local tracks (`track_exists_on_filesystem`, `file://` paths only) so a search never creates lyric files or directories for an absent path. `LocalFileSource::search` (`src/sources/localfiles.cpp`) uses it too, otherwise every track without a `.lrc`/`.txt` file logs a "Failed to open lyrics file" warning.
- **Clearing the panel on track change** (`mac/OpenLyricsView.mm` `setNowPlayingTrack:info:`): the displayed lyrics must be dropped when the track handle changes, mirroring `LyricPanel::on_playback_new_track` on Windows. Without it the previous track's lyrics stay on screen whenever the new track has none, because the panel only repaints when a search returns a result. `setNowPlayingDynamicInfo:` does the same for internet radio, where the track handle never changes and new song metadata arrives through `on_playback_dynamic_info_track`.
