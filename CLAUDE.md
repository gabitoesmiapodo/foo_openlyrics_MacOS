...
- **`g_exists` across the binary boundary** (`src/tag_util.cpp` `g_exists_safe`): `filesystem::g_exists` can throw `exception_io_not_found` across the foobar2000.app/component boundary instead of returning `false` (typeinfo mismatch), so existence checks route through `g_exists_safe`, which treats a throw as "does not exist". Auto-search is skipped for missing local tracks (`track_exists_on_filesystem`, `file://` paths only) so a search never creates lyric files or directories for an absent path.
